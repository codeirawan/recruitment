...
@section('content')
<form class="kt-form" id="kt_form_1" action="{{ route('supplier.update', $supplier->id) }}" method="POST">
    @method('PUT')
    @csrf
                ...
                <div class="kt-section__body">
                    ...
                    <div class="row">
                        <div class="form-group col-sm-6">
                            ...
                            <input id="nama" name="nama" type="text" class="form-control @error('nama') is-invalid @enderror" required value="{{ old('nama', $supplier->name) }}">
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <select id="jenis" name="jenis" class="form-control kt_selectpicker @error('jenis') is-invalid @enderror" required data-live-search="true" title="{{ __('Choose') }} {{ __('Type') }}">
                                <option value="Individual" {{ old('jenis', $supplier->type) == 'Individual' ? 'selected' : '' }}>{{ __('Individual') }}</option>
                                <option value="PT" {{ old('jenis', $supplier->type) == 'PT' ? 'selected' : '' }}>PT</option>
                                <option value="CV" {{ old('jenis', $supplier->type) == 'CV' ? 'selected' : '' }}>CV</option>
                            </select>
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <input id="nomor_telepon" name="nomor_telepon" type="text" class="form-control @error('nomor_telepon') is-invalid @enderror" required value="{{ old('nomor_telepon', $supplier->telephone) }}">
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <input id="npwp" name="npwp" type="text" class="form-control @error('npwp') is-invalid @enderror" required value="{{ old('npwp', $supplier->npwp) }}">
                        ...
                        <div class="form-group col-sm-12">
                            ...
                            <textarea id="alamat" name="alamat" class="form-control @error('alamat') is-invalid @enderror" required>{{ old('alamat', $supplier->address) }}</textarea>
                            ...
                            <table class="table table-striped- table-bordered table-hover" id="kt_table_1">
                                <tbody>
                                    @foreach ($products as $i => $product)
                                        <tr>
                                            <td><a href="{{ $product->photo }}" class="kt-media kt-media--lg" target="_blank"><img src="{{ $product->photo }}"></a></td>
                                            <td>{{ $product->code }}</td>
                                            <td>{{ $product->name }}</td>
                                            <td>{{ $product->category_name . ' (' . $product->category_code . ')' }}</td>
                                            <td><a id="btn-delete" href="javascript:;" class="btn btn-sm btn-clean btn-icon btn-icon-md btn-tooltip" title="{{ __('Delete')}}"><i class="la la-trash"></i></a><input type="hidden" name="barang[{{ $i }}]" value="{{ $product->id }}"></td>
                                            <td>{{ $product->id }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <script type="text/javascript">
        ...
        var table = $('#kt_table_1').DataTable({
            ...
            columns: [
                ...
                { title: "Product ID", defaultContent: '-', visible: false },
            ],
        ...
        var productCount = table.column( 0 ).data().length;
