...
@section('title')
    {{ __('Create') }} {{ __('Product') }} | {{ config('app.name') }}
@endsection
...
@section('breadcrumb')
    ...
    <span class="kt-subheader__breadcrumbs-separator"></span><a href="{{ ('product/create') }}" class="kt-subheader__breadcrumbs-link">{{ __('Create') }} {{ __('Product') }}</a>
@endsection

@section('content')
<form class="kt-form" id="kt_form_1" action="{{ ('product/store') }}" method="POST" enctype="multipart/form-data">
    @csrf

    <div class="kt-portlet" id="kt_page_portlet">
        <div class="kt-portlet__head kt-portlet__head--lg">
            <div class="kt-portlet__head-label">
                <h3 class="kt-portlet__head-title">{{ __('Create') }} {{ __('Product') }}</h3>
            </div>
                    ...
                    <div class="row">
                        <div class="form-group col-sm-6">
                            ...
                            <div class="input-group">
                                ...
                                <input id="kode" name="kode" type="text" class="form-control @error('kode') is-invalid @enderror" required value="{{ old('kode') }}">
                            </div>
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <select id="kategori" name="kategori" class="form-control kt_selectpicker @error('kategori') is-invalid @enderror" required data-live-search="true" title="{{ __('Choose') }} {{ __('Category') }}">
                                @foreach ($productCategories as $productCategory)
                                    <option value="{{ $productCategory->id }}" data-code="{{ $productCategory->code }}" {{ old('kategori') == $productCategory->id ? 'selected' : '' }}>{{ $productCategory->name }} ({{ $productCategory->code }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            <select id="satuan" name="satuan" class="form-control kt_selectpicker @error('satuan') is-invalid @enderror" required data-live-search="true" title="{{ __('Choose') }} {{ __('Unit') }}">
                                @foreach ($productUnits as $productUnit)
                                    <option value="{{ $productUnit->id }}" {{ old('satuan') == $productUnit->id ? 'selected' : '' }}>{{ $productUnit->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <span class="kt-switch kt-switch--sm kt-switch--icon kt-switch--primary kt-switch--outline">
                                <label class="mb-0">
                                    <input type="checkbox" id="sku_per_satuan" name="sku_per_satuan" {{ old('sku_per_satuan', true) ? 'checked' : '' }}>
                                    <span class="m-0"></span>
                                </label>
                            </span>
                        </div>
                        ...
                        <div class="form-group col-sm-3">
                            ...
                            <span class="kt-switch kt-switch--sm kt-switch--icon kt-switch--primary kt-switch--outline">
                                <label class="mb-0">
                                    <input type="checkbox" id="alamat_mac" name="alamat_mac" {{ old('alamat_mac', true) ? 'checked' : '' }}>
                                    <span class="m-0"></span>
                                </label>
                            </span>
                        </div>
